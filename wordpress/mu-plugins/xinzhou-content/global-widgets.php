    ...
    protected function render(): void {
        ...
        <div class="xz-page-header-shell">
            ...
            <header class="xz-page-header"><div class="xz-page-header__inner">
                ...
                <nav class="xz-page-header__nav" aria-label="Primary navigation"><?php foreach ($items as $item) : $products = str_contains(strtolower((string) $item->title), 'product'); $active = $is_active($item); if ($products) : ?><div class="xz-page-header__products"><a class="<?php echo $active ? 'is-active' : ''; ?>" href="<?php echo esc_url($item->url); ?>"<?php echo $active ? ' aria-current="page"' : ''; ?>><?php echo esc_html($item->title); ?><svg class="xz-page-header__products-icon" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="m4 6 4 4 4-4" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg></a><div class="xz-page-header__dropdown" aria-label="Product categories"><?php foreach ($terms as $term) : $term_link = get_term_link($term); if (is_wp_error($term_link)) { continue; } ?><a href="<?php echo esc_url($term_link); ?>"><?php echo esc_html($term->name); ?></a><?php endforeach; ?></div></div><?php else : ?><a class="<?php echo $active ? 'is-active' : ''; ?>" href="<?php echo esc_url($item->url); ?>"<?php echo $active ? ' aria-current="page"' : ''; ?>><?php echo esc_html($item->title); ?></a><?php endif; endforeach; ?></nav>
